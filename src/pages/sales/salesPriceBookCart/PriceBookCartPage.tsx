...
import { updateCartItem, removeFromCart, getDeliveryCharge, placeOrder, getSalesWarehouseProfile, clearCart as clearCartApi } from '../../../redux/apis/sales/salesOrderApis';
...
import { fetchSalesPriceBookCartItems, clearSalesPriceBookCart } from '../../../redux/slices/salesPriceBookCartSlice';
...
const PriceBookCartPage: React.FC = () => {
  ...
  const { items: cartItems, userLimitMinOrderAmount, totalAmountWithTax, totalAmount } = useSelector((state: RootState) => state.salesPriceBookCart) as any;
  const addresses = useSelector((state: any) => state.auth);
  const selectedCustomer = addresses?.selectedCustomer;
  ...
  const handleClearAllCartItems = async () => {
    try {
      ...
      await dispatch(clearSalesPriceBookCart());
...
export default PriceBookCartPage;
