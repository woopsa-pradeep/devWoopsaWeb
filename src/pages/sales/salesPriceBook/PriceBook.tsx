...
import { getPriceBookInventoryItems, addToCart, updateCartItem, removeFromCart, clearCart, getSalesCategoryPriceClassByCustomer, getSalesCategoryByCustomer } from '../../../redux/apis/sales/salesOrderApis';
...
import { fetchSalesPriceBookCartItems, setSalesPriceBookCartCount } from '../../../redux/slices/salesPriceBookCartSlice';
...
const PriceBook = () => {
  ...
  const cartValidationData = useAppSelector((state: any) => state.salesPriceBookCart);
  ...
  // price (Product.Price_With_Tax) because it already includes any quantity/case discounts.
  // Otherwise, we can accidentally use the original non-discounted base price.
  ...
  // Keep global retailer cart count (used by Navbar badge) in sync with local orderItems
  useEffect(() => {
    dispatch(setSalesPriceBookCartCount(Object.keys(orderItems).length));
  }, [dispatch, orderItems]);
  ...
  const handleManualQuantityChange = (id: string, newQuantity: number) => {
    ...
    // Keep cartItemsData in sync so Order Details has full product (e.g. prepaidTaxRate) when qty changed via text input
  ...
  const handleDiscountConfirm = (discountInfo: any) => {
    if (selectedDiscountProduct && discountInfo) {
      try {
        ...
        (product as any).discountedBasePrice = discountedBasePrice;
        
        // Calculate Price_With_Tax from discounted base price: (discountedBasePrice + Tax_Rate) * (1 + prepaidTaxRate)
        const basePriceWithTax = Number(Number(discountedBasePrice + taxRate).toFixed(2));
        const payloadPriceWithTax = roundAmount(basePriceWithTax * (1 + prepaidTaxRate)); // includes prepaid tax (expected by calculateCartPayload)
        ...
        setTimeout(async () => {
          try {
            ...
            const response = await addToCart(auth?.selectedCustomer?.C_Number?.toString() || '', {
              Item_Number: parseInt(product.id),
              ...payload
            });
            // Store cart item id from response so subsequent updates use updateCartItem
      ...
      try {
        ...
        setOrderItems(prev => ({
          ...
          [product.id]: {
            quantity: quantity,
            Description: product.name,
            price: getDisplayUnitPrice(product), // Prepaid-tax aware display price
            productId: prev[product.id]?.productId || 0,
            placedBySalesPerson: false
          }
        }));
  ...
  const getInventoryData = useCallback(async () => {
    ...
    try {
      ...
      const response:any = await getPriceBookInventoryItems((auth?.selectedCustomer?.C_Number || auth?.storeDetail?.C_Number || '').toString(), params, abortController.signal);
  ...
  const loadCartItems = useCallback(async () => {
    ...
    try {
      const result = await dispatch(fetchSalesPriceBookCartItems());
      if (fetchSalesPriceBookCartItems.fulfilled.match(result)) {
  ...
  const handleQuantityChange = async (id: string, change: number) => {
    ...
    if (newQuantity <= 0) {
      ...
      try {
        ...
        if (productId) {
          await removeFromCart(String(productId), auth?.selectedCustomer?.C_Number?.toString() || '');
        }
        setOrderItems(prev => {
          const {...rest} = prev;
          delete rest[id];
    ...
    quantityDebounceRef.current[id] = setTimeout(async () => {
      ...
      try {
        ...
        if (currentItem?.productId) {
          await updateCartItem(String(currentItem.productId), payload, auth?.selectedCustomer?.C_Number?.toString() || '');
        } else {
          const response = await addToCart(auth?.selectedCustomer?.C_Number?.toString() || '', {
  ...
  const handleConfirmClear = async () => {
    ...
    try {
      await clearCart(auth?.selectedCustomer?.C_Number?.toString() || '');
      setOrderItems({});
      setCartItemsData({});
    ...
    try {
      ...
      for (const item of priceChangeItems) {
        ...
        if (productData) {
          ...
          await updateCartItem(String(item.Product.id), {
            ...payload,
            originalPrice: item.newPrice
          }, auth?.selectedCustomer?.C_Number?.toString() || '');
  ...
  const handleInactiveItemsClose = async () => {
    ...
    try {
      ...
      for (const item of inactiveItems) {
        if (item.Product?.id) {
          await removeFromCart(String(item.Product.id), auth?.selectedCustomer?.C_Number?.toString() || '');
          const id = item.Item_Number?.toString();
  ...
  const processQrCode = async (upcCode: string) => {
    try {
      ...
      const response: any = await getPriceBookInventoryItems((auth?.selectedCustomer?.C_Number || auth?.storeDetail?.C_Number || '').toString(), params);
      ...
      try {
        ...
        if (existingItem?.productId) {
          await updateCartItem(String(existingItem.productId), payload, auth?.selectedCustomer?.C_Number?.toString() || '');
        } else {
          const response = await addToCart(auth?.selectedCustomer?.C_Number?.toString() || '', {
  ...
  const handleKeyboardProductSelect = (product: Product, quantity: number) => {
    ...
    setOrderItems(prev => ({
      ...
      [product.id]: {
        quantity: newQuantity,
        Description: product.name,
        price: getDisplayUnitPrice(product),
        productId: prev[product.id]?.productId || 0,
        placedBySalesPerson: false // Default to false for retailer orders
      }
    }));
    // Keep cartItemsData in sync so Order Details has full product (e.g. prepaidTaxRate)
    setCartItemsData(prev => ({ ...prev, [product.id]: product }));
    // Add to insertion order if it's a new item
    if (isNewItem) {
          setItemInsertionOrder(prev => [product.id, ...prev]);
    }
          ...
          <OrderDetails
            items={itemInsertionOrder
              ...
              .map((id) => {
                ...
                let prepaidTaxRate = Number(productData?.prepaidTaxRate) || 0;
                ...
                const hasDiscountedPrice =
                  productData && Math.abs(Number(item.price) - Number(originalPriceWithTax)) > 0.005;
