...
import { getOrderDeliveryStatus, getOrderHistoryByOrderNumber, getOrderPdf } from '../../../redux/apis/sales/salesOrderApis';
...
const OrderDetailsPage = () => {
  ...
  const getOrderDeliveryStatusApi = async () => {
    const response: any = await getOrderDeliveryStatus(orderId, selectedCustomer?.C_Number) as any;
  ...
  const columns: TableColumn<any>[] = [
    ...
    //   id: 'subtotal', 
    //   label: 'Subtotal', 
    //   render: (row) => row.showWithOutPrice ? '-' : `$${Number(row.Price * row.Quantity_Ordered).toFixed(2)}`, align: 'right'
    // },
    // { 
    //   id: 'discount', 
    //   label: 'Discount', 
    //   render: (row) => row.showWithOutPrice ? '-' : `$${Number(row.OffInvoice_Amount || 0).toFixed(2)}`, align: 'right'
    // },
    { 
      id: 'totalPrice', 
      label: 'Total Price', 
    ...
    <Box sx={{ padding: '20px' }}>
      ...
      <Box display="flex" alignItems="center" gap={1} mb={3}>
        <KeyboardBackspaceOutlined 
          onClick={() => navigate('/sales/order/history')}
          sx={{ cursor: 'pointer' }}
        />
        <Typography fontSize={20} fontWeight={500}>Order Details</Typography>
      </Box>
