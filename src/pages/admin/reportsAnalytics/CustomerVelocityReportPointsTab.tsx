...
import { getCustomerVelocityReportPoints } from '../../../redux/apis/distrubutor/reportsApis';
...
const REPORT_TITLE = 'Velocity Report - Customer (Points: Sales Detail)';

const CustomerVelocityReportPointsTab: React.FC = () => {
  ...
  const handlePreview = useCallback(async () => {
    ...
    try {
      ...
      const response = (await getCustomerVelocityReportPoints(filters.startDate, filters.endDate)) as any;
      ...
      console.error('Error loading Velocity Report - Customer (Points: Sales Detail):', error);
  ...
  const handleGenerateCSV = useCallback(() => {
    ...
    try {
      ...
      link.download = `velocity-report-customer-points-sales-detail-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const handleGeneratePDF = useCallback(async () => {
    ...
    try {
      ...
      doc.text(REPORT_TITLE, pageWidth / 2, startY - 4, { align: 'center' });
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      if (dateRangeText) {
      ...
      doc.save(`velocity-report-customer-points-sales-detail-${new Date().toISOString().split('T')[0]}.pdf`);
...
export default CustomerVelocityReportPointsTab;
