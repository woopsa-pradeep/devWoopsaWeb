...
import { getCustomerLastSaleReport } from '../../../redux/apis/distrubutor/reportsApis';
...
const CustomerLastSaleReportTab: React.FC = () => {
  ...
  const handlePreview = useCallback(async () => {
    ...
    try {
      ...
      const response = (await getCustomerLastSaleReport(
        filters.startDate,
        filters.endDate,
      )) as any;
      const raw =
      ...
      const mapped: LastSaleRow[] = filtered.map((row: any) => {
        ...
        const lastInvoiceDateRaw =
          row['orderHeaders.Invoice_Date'] ??
          row.Invoice_Date ??
          row.LastInvoiceDate ??
          row.InvoiceDate ??
          row.Last_Invoice_Date ??
          row.LastInvoice_Date;
        const lastInvoiceNumber =
          row['orderHeaders.Document_Number'] ??
          row['orderHeaders.Invoice_Number'] ??
          row.Invoice_Number ??
          row.LastInvoiceNumber ??
          row.InvoiceNumber ??
          row.Last_Invoice_Number;
        const lastInvoiceTotal =
          row['orderHeaders.Invoice_Total'] ??
          row.Invoice_Total ??
          row.LastInvoiceTotal ??
          row.InvoiceTotal ??
          row.Last_Invoice_Total ??
        ...
        return {
          ...
          zip,
          phone,
          lastInvoiceDate: lastInvoiceDateRaw
            ? formatApiDate(String(lastInvoiceDateRaw))
            : undefined,
          lastInvoiceNumber,
          lastInvoiceTotal:
            typeof lastInvoiceTotal === 'number' ? lastInvoiceTotal : Number(lastInvoiceTotal) || 0,
        };
      });
      ...
      console.error('Error loading Customer Last Sale report:', error);
      toast.error('Failed to load Customer Last Sale report.');
    ...
    try {
      ...
      rows.forEach((r) => {
        const vals = [
          ...
          r.lastInvoiceDate ?? '',
          r.lastInvoiceNumber ?? '',
          (r.lastInvoiceTotal ?? 0).toFixed(2),
        ];
      ...
      const filename = `customer-last-sale-${timestamp}.csv`;
  ...
  const handleGeneratePDF = useCallback(async () => {
    ...
    try {
      ...
      doc.text('Sales History (Customer Last Sale)', pageWidth / 2, headerBottomY - 4, {
        align: 'center',
      });
      ...
      const timestamp = new Date().toISOString().split('T')[0];
      doc.save(`customer-last-sale-${timestamp}.pdf`);
    ...
    >
      ...
      >
        {!showPreview && (
          ...
          >
            Customer Last Sale Report Configuration
          </Typography>
        )}
...
export default CustomerLastSaleReportTab;
