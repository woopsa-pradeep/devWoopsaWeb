...
const AddUpdateRequest: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (isEditMode && id) {
        await updateRetailerRequest(id, data);
        toast.success('Retailer request updated successfully');
      } else {
  ...
  const steps = [
    {
      label: 'Basic Information & Contact',
      content: (
        <Box sx={{ py: 2 }}>
    ...
    {
      label: 'Financial & Additional Info',
      content: (
        <Box sx={{ py: 2 }}>
  ...
  return (
    <Box>
      {/* Header */}
      <Box display="flex" alignItems="center" mb={1} sx={{ py: { xs: 1, md: 2 } }}>
        <IconButton 
        ...
        >
          {isEditMode ? 'Edit Retailer Request' : 'Add New Retailer Request'}
        </Typography>
      </Box>
