...
import { customerList, updateCustomer, getCustomerById, listOfCustomersCreate, uploadRetailerDocuments, updateRetailerDocuments, uploadImages } from '../../../redux/apis/distrubutor/retailerApis';
...
const Retailer = () => {
  ...
  const [loadingFullDetails, setLoadingFullDetails] = useState<{ [key: number]: boolean }>({});

  // Inline editing state
  const [editingRetailers, setEditingRetailers] = useState<{ [key: string]: boolean }>({});
  const [retailerEditData, setRetailerEditData] = useState<{ [key: string]: any }>({});
  const [retailerOriginalData, setRetailerOriginalData] = useState<{ [key: string]: any }>({});
  const [savingRetailer, setSavingRetailer] = useState<{ [key: string]: boolean }>({});
  ...
  const [geocodingState, setGeocodingState] = useState<{ [key: string]: { lat: number | null; long: number | null; loading: boolean } }>({});
  
  // Document state per retailer
  const [documentState, setDocumentState] = useState<{ [key: string]: {
    attachments: File[];
    ...
    if (viewMode === 'detailed') {
      ...
      const fetchDetailedRetailers = async () => {
        ...
        try {
          ...
          const total = res?.data?.totalCount || res?.data?.data?.totalCount || 0;
    ...
    if (viewMode === 'detailed') {
      ...
      Object.keys(expandedCards).forEach((cardId) => {
        if (expandedCards[cardId]) {
          ...
          if (customerNum && !detailedCustomersWithFullData[customerNum] && !loadingFullDetails[customerNum]) {
            ...
            if (customer) {
              // Set loading state
              setLoadingFullDetails(prev => ({ ...prev, [customerNum]: true }));
              
              const ignore = false;
              const fetchFullDetailsForCustomer = async () => {
                try {
                  console.log('Fetching full details for customer:', customerNumber);
                  const response = await getCustomerById(customerNumber) as any;
                  ...
                  console.error('Error fetching full details for customer:', error);
  ...
  }, [expandedCards, viewMode, detailedData, detailedCustomersWithFullData, loadingFullDetails]);
    ...
    try {
      ...
      setRetailerDropdownOptions({
        salesRep: (data.salesRep || []).map((sr: any) => ({
          label: `${sr.S_Number || ''} - ${sr.S_Desc || ''}`.trim() || `Sales Rep ${sr.S_Number || ''}`,
        ...
        invoiceFormat: (data.documentAdditionalFormat || []).map((ifmt: any) => ({
        ...
        customerStatus: (data.customerStatus || []).map((status: any) => ({
          label: `${status.C_Status || ''} - ${status.C_StatusDescription || ''}`.trim() || `Status ${status.C_StatusCode || ''}`,
  ...
  const handleEditRetailer = (customerNumber: string | number, retailerData: any) => {
    ...
    setRetailerEditData(prev => ({
      ...
      [customerNumber]: {
        ...
        // Sales Rep & Trade
        C_Salesman: dataToUse.C_Salesman?.toString() || retailerData.salesRep?.S_Number?.toString() || retailerData.C_Salesman?.toString() || '',
        C_ClassOfTrade: dataToUse.C_ClassOfTrade || retailerData.classOfTrade?.Trade_Code || retailerData.C_ClassOfTrade || '000',
        // Operations
        C_OperationHours1: dataToUse.C_OperationHours1 !== undefined ? dataToUse.C_OperationHours1 : (retailerData.C_OperationHours1 || 0),
        C_OperationHours2: dataToUse.C_OperationHours2 !== undefined ? dataToUse.C_OperationHours2 : (retailerData.C_OperationHours2 || 0),
        C_Inactive: dataToUse.C_Inactive !== undefined ? (dataToUse.C_Inactive === true || dataToUse.C_Inactive === 1 ? 1 : 0) : (retailerData.C_Inactive !== undefined ? (retailerData.C_Inactive === true || retailerData.C_Inactive === 1 ? 1 : 0) : 0),
        C_StatusCode: dataToUse.C_StatusCode !== undefined ? dataToUse.C_StatusCode : (retailerData.C_StatusCode !== undefined ? retailerData.C_StatusCode : 0),
        C_OrderDay: dataToUse.C_OrderDay !== undefined ? dataToUse.C_OrderDay : (retailerData.C_OrderDay || 0),
        C_OrderDaySequence: dataToUse.C_OrderDaySequence !== undefined ? dataToUse.C_OrderDaySequence : (retailerData.C_OrderDaySequence || 1),
        // Delivery
        Delivery_Charge: dataToUse.Delivery_Charge !== undefined ? (dataToUse.Delivery_Charge === true || dataToUse.Delivery_Charge === 1 ? 1 : 0) : (retailerData.Delivery_Charge !== undefined ? (retailerData.Delivery_Charge === true || retailerData.Delivery_Charge === 1 ? 1 : 0) : 0),
        Delivery_Amount: dataToUse.Delivery_Amount !== undefined ? dataToUse.Delivery_Amount : (retailerData.Delivery_Amount || 0),
        Delivery_ID: dataToUse.Delivery_ID !== undefined ? dataToUse.Delivery_ID : (retailerData.Delivery_ID !== undefined ? retailerData.Delivery_ID : 0),
        // Pricing & Discounts
        C_CaseDiscount: dataToUse.C_CaseDiscount !== undefined ? (dataToUse.C_CaseDiscount === true || dataToUse.C_CaseDiscount === 1 ? 1 : 0) : (retailerData.C_CaseDiscount !== undefined ? (retailerData.C_CaseDiscount === true || retailerData.C_CaseDiscount === 1 ? 1 : 0) : 0),
        // Invoice & EDI
        C_InvoiceFormat: dataToUse.C_InvoiceFormat !== undefined ? dataToUse.C_InvoiceFormat : (retailerData.C_InvoiceFormat !== undefined ? retailerData.C_InvoiceFormat : 0),
        EDI_Format: dataToUse.EDI_Format !== undefined ? dataToUse.EDI_Format : (retailerData.EDI_Format !== undefined ? retailerData.EDI_Format : 0),
        // Terms & Credit
        TermCode: dataToUse.TermsCode !== undefined ? dataToUse.TermsCode : (dataToUse.TermCode !== undefined ? dataToUse.TermCode : (retailerData.terms?.TermsCode !== undefined ? retailerData.terms.TermsCode : (retailerData.TermCode !== undefined ? retailerData.TermCode : 0))),
        Credit_Limit: dataToUse.Credit_Limit !== undefined ? dataToUse.Credit_Limit : (retailerData.Credit_Limit || 0),
        // Additional
        C_Memo: dataToUse.C_Memo || retailerData.C_Memo || '',
        Service_Charge: dataToUse.Service_Charge !== undefined ? (dataToUse.Service_Charge === true || dataToUse.Service_Charge === 1 ? 1 : 0) : (retailerData.Service_Charge !== undefined ? (retailerData.Service_Charge === true || retailerData.Service_Charge === 1 ? 1 : 0) : 0),
        Other_Amount: dataToUse.Other_Amount !== undefined ? dataToUse.Other_Amount : (retailerData.Other_Amount || 0),
        // POS
        POS_CashC: dataToUse.POS_CashC !== undefined ? (dataToUse.POS_CashC === true || dataToUse.POS_CashC === 1 ? 1 : 0) : (retailerData.POS_CashC !== undefined ? (retailerData.POS_CashC === true || retailerData.POS_CashC === 1 ? 1 : 0) : 1),
        POS_CheckC: dataToUse.POS_CheckC !== undefined ? (dataToUse.POS_CheckC === true || dataToUse.POS_CheckC === 1 ? 1 : 0) : (retailerData.POS_CheckC !== undefined ? (retailerData.POS_CheckC === true || retailerData.POS_CheckC === 1 ? 1 : 0) : 1),
        POS_CreditC: dataToUse.POS_CreditC !== undefined ? (dataToUse.POS_CreditC === true || dataToUse.POS_CreditC === 1 ? 1 : 0) : (retailerData.POS_CreditC !== undefined ? (retailerData.POS_CreditC === true || retailerData.POS_CreditC === 1 ? 1 : 0) : 1),
        POS_DebitC: dataToUse.POS_DebitC !== undefined ? (dataToUse.POS_DebitC === true || dataToUse.POS_DebitC === 1 ? 1 : 0) : (retailerData.POS_DebitC !== undefined ? (retailerData.POS_DebitC === true || retailerData.POS_DebitC === 1 ? 1 : 0) : 1),
        POS_OtherC: dataToUse.POS_OtherC !== undefined ? (dataToUse.POS_OtherC === true || dataToUse.POS_OtherC === 1 ? 1 : 0) : (retailerData.POS_OtherC !== undefined ? (retailerData.POS_OtherC === true || retailerData.POS_OtherC === 1 ? 1 : 0) : 1),
        POS_HouseC: dataToUse.POS_HouseC !== undefined ? (dataToUse.POS_HouseC === true || dataToUse.POS_HouseC === 1 ? 1 : 0) : (retailerData.POS_HouseC !== undefined ? (retailerData.POS_HouseC === true || retailerData.POS_HouseC === 1 ? 1 : 0) : 1),
        // MSA
        MSA_AcceptPromo: dataToUse.MSA_AcceptPromo !== undefined ? (dataToUse.MSA_AcceptPromo === true || dataToUse.MSA_AcceptPromo === 1 ? 1 : 0) : (retailerData.MSA_AcceptPromo !== undefined ? (retailerData.MSA_AcceptPromo === true || retailerData.MSA_AcceptPromo === 1 ? 1 : 0) : 1),
        // Email Settings
        emailInvoice: dataToUse.emailInvoice !== undefined ? (dataToUse.emailInvoice === true || dataToUse.emailInvoice === 1 ? 1 : 0) : (retailerData.emailInvoice !== undefined ? (retailerData.emailInvoice === true || retailerData.emailInvoice === 1 ? 1 : 0) : 0),
        emailInvoiceEDI: dataToUse.emailInvoiceEDI !== undefined ? (dataToUse.emailInvoiceEDI === true || dataToUse.emailInvoiceEDI === 1 ? 1 : 0) : (retailerData.emailInvoiceEDI !== undefined ? (retailerData.emailInvoiceEDI === true || retailerData.emailInvoiceEDI === 1 ? 1 : 0) : 0),
        emailReport: dataToUse.emailReport !== undefined ? (dataToUse.emailReport === true || dataToUse.emailReport === 1 ? 1 : 0) : (retailerData.emailReport !== undefined ? (retailerData.emailReport === true || retailerData.emailReport === 1 ? 1 : 0) : 0),
        emailStatement: dataToUse.emailStatement !== undefined ? (dataToUse.emailStatement === true || dataToUse.emailStatement === 1 ? 1 : 0) : (retailerData.emailStatement !== undefined ? (retailerData.emailStatement === true || retailerData.emailStatement === 1 ? 1 : 0) : 0),
        emailPromo: dataToUse.emailPromo !== undefined ? (dataToUse.emailPromo === true || dataToUse.emailPromo === 1 ? 1 : 0) : (retailerData.emailPromo !== undefined ? (retailerData.emailPromo === true || retailerData.emailPromo === 1 ? 1 : 0) : 0),
        // Category Allow
        Category_Allow01: dataToUse.Category_Allow01 !== undefined ? (dataToUse.Category_Allow01 === true || dataToUse.Category_Allow01 === 1 ? 1 : 0) : (retailerData.Category_Allow01 !== undefined ? (retailerData.Category_Allow01 === true || retailerData.Category_Allow01 === 1 ? 1 : 0) : 1),
        Category_Allow02: dataToUse.Category_Allow02 !== undefined ? (dataToUse.Category_Allow02 === true || dataToUse.Category_Allow02 === 1 ? 1 : 0) : (retailerData.Category_Allow02 !== undefined ? (retailerData.Category_Allow02 === true || retailerData.Category_Allow02 === 1 ? 1 : 0) : 1),
        Category_Allow03: dataToUse.Category_Allow03 !== undefined ? (dataToUse.Category_Allow03 === true || dataToUse.Category_Allow03 === 1 ? 1 : 0) : (retailerData.Category_Allow03 !== undefined ? (retailerData.Category_Allow03 === true || retailerData.Category_Allow03 === 1 ? 1 : 0) : 1),
        Category_Allow04: dataToUse.Category_Allow04 !== undefined ? (dataToUse.Category_Allow04 === true || dataToUse.Category_Allow04 === 1 ? 1 : 0) : (retailerData.Category_Allow04 !== undefined ? (retailerData.Category_Allow04 === true || retailerData.Category_Allow04 === 1 ? 1 : 0) : 1),
        Category_Allow05: dataToUse.Category_Allow05 !== undefined ? (dataToUse.Category_Allow05 === true || dataToUse.Category_Allow05 === 1 ? 1 : 0) : (retailerData.Category_Allow05 !== undefined ? (retailerData.Category_Allow05 === true || retailerData.Category_Allow05 === 1 ? 1 : 0) : 1),
        Category_Allow06: dataToUse.Category_Allow06 !== undefined ? (dataToUse.Category_Allow06 === true || dataToUse.Category_Allow06 === 1 ? 1 : 0) : (retailerData.Category_Allow06 !== undefined ? (retailerData.Category_Allow06 === true || retailerData.Category_Allow06 === 1 ? 1 : 0) : 1),
        Category_Allow07: dataToUse.Category_Allow07 !== undefined ? (dataToUse.Category_Allow07 === true || dataToUse.Category_Allow07 === 1 ? 1 : 0) : (retailerData.Category_Allow07 !== undefined ? (retailerData.Category_Allow07 === true || retailerData.Category_Allow07 === 1 ? 1 : 0) : 1),
        Category_Allow08: dataToUse.Category_Allow08 !== undefined ? (dataToUse.Category_Allow08 === true || dataToUse.Category_Allow08 === 1 ? 1 : 0) : (retailerData.Category_Allow08 !== undefined ? (retailerData.Category_Allow08 === true || retailerData.Category_Allow08 === 1 ? 1 : 0) : 1),
        Category_Allow09: dataToUse.Category_Allow09 !== undefined ? (dataToUse.Category_Allow09 === true || dataToUse.Category_Allow09 === 1 ? 1 : 0) : (retailerData.Category_Allow09 !== undefined ? (retailerData.Category_Allow09 === true || retailerData.Category_Allow09 === 1 ? 1 : 0) : 1),
        Category_Allow10: dataToUse.Category_Allow10 !== undefined ? (dataToUse.Category_Allow10 === true || dataToUse.Category_Allow10 === 1 ? 1 : 0) : (retailerData.Category_Allow10 !== undefined ? (retailerData.Category_Allow10 === true || retailerData.Category_Allow10 === 1 ? 1 : 0) : 1),
        Category_Allow11: dataToUse.Category_Allow11 !== undefined ? (dataToUse.Category_Allow11 === true || dataToUse.Category_Allow11 === 1 ? 1 : 0) : (retailerData.Category_Allow11 !== undefined ? (retailerData.Category_Allow11 === true || retailerData.Category_Allow11 === 1 ? 1 : 0) : 1),
        Category_Allow12: dataToUse.Category_Allow12 !== undefined ? (dataToUse.Category_Allow12 === true || dataToUse.Category_Allow12 === 1 ? 1 : 0) : (retailerData.Category_Allow12 !== undefined ? (retailerData.Category_Allow12 === true || retailerData.Category_Allow12 === 1 ? 1 : 0) : 1),
      }
    }));
  };
console.log(handleEditRetailer);
  ...
  const handleSaveRetailer = async (customerNumber: string | number) => {
    ...
    try {
      ...
      if (transformedOriginalData.Jurisdiction_State !== undefined) {
        transformedOriginalData.Jurisdiction_State = String(transformedOriginalData.Jurisdiction_State);
      } else {
        transformedOriginalData.Jurisdiction_State = '0';
      }
      if (transformedOriginalData.Jurisdiction_County !== undefined) {
        transformedOriginalData.Jurisdiction_County = String(transformedOriginalData.Jurisdiction_County);
      } else {
        transformedOriginalData.Jurisdiction_County = '0';
      }
      if (transformedOriginalData.Jurisdiction_City !== undefined) {
        transformedOriginalData.Jurisdiction_City = String(transformedOriginalData.Jurisdiction_City);
      } else {
      ...
      if (transformedOriginalData.C_Salesman !== undefined) {
        transformedOriginalData.C_Salesman = String(transformedOriginalData.C_Salesman);
      } else if (originalData.salesRep?.S_Number !== undefined) {
        transformedOriginalData.C_Salesman = String(originalData.salesRep.S_Number);
      } else {
      ...
      const response = await updateCustomer(String(customerNumber), changedFields) as any;
      if (response?.success) {
        // Upload documents if any (same as AddRetailer.tsx)
        const docState = documentState[customerNumber];
        if (docState && (docState.attachments.length > 0 || docState.salesTaxDoc || docState.CigTaxDoc || docState.licenseAttachments.length > 0 || docState.feinDocument)) {
          try {
            const documentData: {
              attachments?: string[];
              salesTaxDoc?: string | null;
              ...
              licenseAttachments: [...(docState.existingUrls.licenseAttachments || [])],
            ...
            if (docState.salesTaxDoc) {
              documentData.salesTaxDoc = await uploadFileAndGetUrl(docState.salesTaxDoc);
            }

            // Upload new CigTaxDoc
            if (docState.CigTaxDoc) {
              documentData.CigTaxDoc = await uploadFileAndGetUrl(docState.CigTaxDoc);
            }

            // Upload new licenseAttachments
            if (docState.licenseAttachments.length > 0) {
              const uploadedUrls = await Promise.all(
                docState.licenseAttachments.map(file => uploadFileAndGetUrl(file))
              );
            ...
            if (docState.feinDocument) {
              documentData.feinDocument = await uploadFileAndGetUrl(docState.feinDocument);
            }
        ...
        if (updatedRetailer) {
          ...
          setDetailedData(prev => {
            const index = prev.findIndex((r: any) => r.C_Number === customerNumber);
            if (index >= 0) {
              const newData = [...prev];
  ...
  const updateRetailerEditField = (customerNumber: string | number, field: string, value: any) => {
    setRetailerEditData(prev => ({
      ...prev,
      [customerNumber]: {
        ...prev[customerNumber],
    ...
    return (
      <Box 
        ...
        sx={{ 
          ...
          '&:hover': {
            bgcolor: isEditing ? 'transparent' : (theme) => theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)'
          }
        }}
      >
        <Typography 
        ...
        {isEditing ? (
          type === 'number' ? (
            <TextInput
              type="number"
              value={editValue || ''}
              onChange={(e) => updateRetailerEditField(customerNumber, field, e.target.value)}
              sx={{ 
                fontSize: 13,
                '& .MuiInputBase-input': { 
                  fontSize: 13,
            ...
            <SwitchInput
              ...
              onChange={(checked) => {
                ...
                const numberFields = ['POS_CashC', 'POS_CheckC', 'POS_CreditC', 'POS_DebitC', 'POS_OtherC', 'POS_HouseC', 
                                      'emailInvoice', 'emailInvoiceEDI', 'emailReport', 'emailStatement', 'emailPromo',
                                      'Category_Allow01', 'Category_Allow02', 'Category_Allow03', 'Category_Allow04',
                                      'Category_Allow05', 'Category_Allow06', 'Category_Allow07', 'Category_Allow08',
                                      'Category_Allow09', 'Category_Allow10', 'Category_Allow11', 'Category_Allow12',
                                      'MSA_AcceptPromo', 'C_Inactive', 'Delivery_Charge', 'C_CaseDiscount', 'Service_Charge'];
                const value = numberFields.includes(field) ? (checked ? 1 : 0) : checked;
  ...
  const renderDetailedCard = (item: any, index: number) => {
    ...
    const retailerDocs = item.retailerDocuments !== null && item.retailerDocuments !== undefined 
              ...
              {editingRetailers[item.C_Number] ? (
                <>
                  <Tooltip title="Save Changes">
                    ...
                    >
                      {savingRetailer[item.C_Number] ? (
                        ...
                        <SaveIcon sx={{ fontSize: { xs: 16, md: 18 } }} color="primary" />
                      )}
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="Cancel Edit">
                    ...
                    >
                      <CancelIcon sx={{ fontSize: { xs: 16, md: 18 } }} color="error" />
                    </IconButton>
                  </Tooltip>
                </>
              ) : (
                ...
                <AccordionDetails sx={{ px: 1, pb: 1, pt: 0.5, flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <Box display="flex" flexDirection="column" gap={1}>
                    ...
                    {renderEditableRetailerField(
                      ...
                      'Sales Rep', 
                      'C_Salesman', 
                      customerData.C_Salesman || item.salesRep?.S_Number || item.C_Salesman, 
                      'dropdown',
                      retailerDropdownOptions.salesRep,
                      (val) => {
                        const option = retailerDropdownOptions.salesRep.find((opt: any) => opt.value === String(val));
                        return option ? option.label : (item.salesRep?.S_Desc || 'N/A');
                      }
                    )}
                    {renderEditableRetailerField(
                      item.C_Number, 
                      'Class of Trade', 
                      'C_ClassOfTrade', 
                      customerData.C_ClassOfTrade || item.classOfTrade?.Trade_Code || item.C_ClassOfTrade, 
                      'dropdown',
                      retailerDropdownOptions.classOfTrade,
                      (val) => {
                        const option = retailerDropdownOptions.classOfTrade.find((opt: any) => opt.value === String(val));
                        return option ? option.label : (item.classOfTrade?.Trade_Desc || 'N/A');
                      }
                    )}
                    {renderEditableRetailerField(
                      item.C_Number, 
                      'Customer Status', 
                      'C_StatusCode', 
                      customerData.C_StatusCode !== undefined ? customerData.C_StatusCode : item.C_StatusCode, 
                      'dropdown',
                      retailerDropdownOptions.customerStatus,
                      (val) => {
                        const option = retailerDropdownOptions.customerStatus.find((opt: any) => opt.value === String(val));
                        return option ? option.label : (item.C_StatusCode || 'N/A');
                      }
                    )}
                    {renderEditableRetailerField(item.C_Number, 'Inactive', 'C_Inactive', customerData.C_Inactive !== undefined ? customerData.C_Inactive : item.C_Inactive, 'toggle')}
                  </Box>
                </AccordionDetails>
              </Accordion>
            </Grid>
              ...
              >
                ...
                <AccordionDetails sx={{ 
                  ...
                  <Box display="flex" flexDirection="column" gap={0.5}>
                    {renderEditableRetailerField(item.C_Number, 'Address', 'C_Address', customerData.C_Address || item.C_Address, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'City', 'C_City', customerData.C_City || item.C_City, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'State', 'C_State', customerData.C_State || item.C_State, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'Zip', 'C_Zip', customerData.C_Zip || item.C_Zip, 'text')}
                    {/* Geocoding Display */}
                    {editingRetailers[item.C_Number] && (() => {
                      const geoState = geocodingState[item.C_Number];
                      const editData = retailerEditData[item.C_Number];
                      const hasAddressFields = editData?.C_Address && editData?.C_City && editData?.C_State && editData?.C_Zip;
                      ...
                      return (
                        ...
                        >
                          <Typography fontSize={13} color="text.secondary" sx={{ minWidth: '140px', fontWeight: 500 }}>
                            Latitude / Longitude
                          </Typography>
                          <Typography fontSize={13} fontWeight={400} color="text.primary" sx={{ flex: 1, textAlign: 'right' }}>
                            {loading ? (
                    ...
                    {renderEditableRetailerField(
                      ...
                      'Jurisdiction State', 
                      'Jurisdiction_State', 
                      customerData.Jurisdiction_State !== undefined ? customerData.Jurisdiction_State : item.Jurisdiction_State, 
                      'dropdown',
                      retailerDropdownOptions.jurisdictionState,
                      (val) => {
                        const option = retailerDropdownOptions.jurisdictionState.find((opt: any) => opt.value === String(val));
                        return option ? option.label : (item.Jurisdiction_State || 'N/A');
                      }
                    )}
                    {renderEditableRetailerField(
                      item.C_Number, 
                      'Jurisdiction County', 
                      'Jurisdiction_County', 
                      customerData.Jurisdiction_County !== undefined ? customerData.Jurisdiction_County : item.Jurisdiction_County, 
                      'dropdown',
                      retailerDropdownOptions.jurisdictionCounty,
                      (val) => {
                        const option = retailerDropdownOptions.jurisdictionCounty.find((opt: any) => opt.value === String(val));
                        return option ? option.label : (item.Jurisdiction_County || 'N/A');
                      }
                    )}
                    {renderEditableRetailerField(
                      item.C_Number, 
                      'Jurisdiction City', 
                      'Jurisdiction_City', 
                      customerData.Jurisdiction_City !== undefined ? customerData.Jurisdiction_City : item.Jurisdiction_City, 
                      'dropdown',
                      retailerDropdownOptions.jurisdictionCity,
                      (val) => {
                        const option = retailerDropdownOptions.jurisdictionCity.find((opt: any) => opt.value === String(val));
                        return option ? option.label : (item.Jurisdiction_City || 'N/A');
                      }
                    )}
                    ...
                    {renderEditableRetailerField(item.C_Number, 'Phone', 'C_Phone', customerData.C_Phone || item.C_Phone, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'Mobile', 'C_PhoneMobile', customerData.C_PhoneMobile || item.C_PhoneMobile, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'Email', 'C_Email', customerData.C_Email || item.C_Email, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'Fax', 'C_Fax', customerData.C_Fax || item.C_Fax, 'text')}
                  </Box>
                </AccordionDetails>
              </Accordion>
            </Grid>
            ...
            <Grid size={{ xs: 12, md: 6, lg: 4 }} sx={{ display: 'flex' }}>
              ...
              >
                ...
                <AccordionDetails sx={{ 
                  ...
                  <Box display="flex" flexDirection="column" gap={0.5}>
                    {renderEditableRetailerField(item.C_Number, 'Sales Tax Number', 'C_SalesTaxNumber', customerData.C_SalesTaxNumber || item.C_SalesTaxNumber, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'Cig License Number', 'C_CigtLicenseNumber', customerData.C_CigtLicenseNumber || item.C_CigtLicenseNumber, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'FEIN', 'C_FEIN', customerData.C_FEIN || item.C_FEIN, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'Other License Number', 'C_OtherLicenseNumber', customerData.C_OtherLicenseNumber || item.C_OtherLicenseNumber, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'Other License Number 2', 'C_OtherLicenseNumber2', customerData.C_OtherLicenseNumber2 || item.C_OtherLicenseNumber2, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'Other License Number 3', 'C_OtherLicenseNumber3', customerData.C_OtherLicenseNumber3 || item.C_OtherLicenseNumber3, 'text')}
                    {renderEditableRetailerField(item.C_Number, 'Credit Limit', 'Credit_Limit', customerData.Credit_Limit !== undefined ? customerData.Credit_Limit : item.Credit_Limit, 'number', undefined, (val) => `$${Number(val || 0).toFixed(2)}`)}
                    {renderEditableRetailerField(
                      item.C_Number, 
                      'Terms', 
                      'TermCode', 
                      customerData.TermsCode !== undefined ? customerData.TermsCode : (item.terms?.TermsCode || item.TermCode), 
                      'dropdown',
                      retailerDropdownOptions.termCode,
                      (val) => {
                        const option = retailerDropdownOptions.termCode.find((opt: any) => opt.value === String(val));
                        return option ? option.label : (item.terms?.Terms || 'N/A');
                      }
                    )}
                    {renderEditableRetailerField(item.C_Number, 'Order Day', 'C_OrderDay', customerData.C_OrderDay !== undefined ? customerData.C_OrderDay : item.C_OrderDay, 'number')}
                    {renderEditableRetailerField(item.C_Number, 'Order Day Sequence', 'C_OrderDaySequence', customerData.C_OrderDaySequence !== undefined ? customerData.C_OrderDaySequence : item.C_OrderDaySequence, 'number')}
                    ...
                    {renderEditableRetailerField(
                      ...
                      'Delivery ID', 
                      'Delivery_ID', 
                      customerData.Delivery_ID !== undefined ? customerData.Delivery_ID : item.Delivery_ID, 
                      'dropdown',
                      retailerDropdownOptions.deliveryId,
                      (val) => {
                        const option = retailerDropdownOptions.deliveryId.find((opt: any) => opt.value === String(val));
                        return option ? option.label : (item.Delivery_ID || 'N/A');
                      }
                    )}
                    ...
                    {renderEditableRetailerField(item.C_Number, 'Service Charge', 'Service_Charge', customerData.Service_Charge !== undefined ? (customerData.Service_Charge === true || customerData.Service_Charge === 1) : (item.Service_Charge !== undefined ? (item.Service_Charge === true || item.Service_Charge === 1) : false), 'toggle')}
                    ...
                    {renderEditableRetailerField(
                      ...
                      'Invoice Format', 
                      'C_InvoiceFormat', 
                      customerData.C_InvoiceFormat !== undefined ? customerData.C_InvoiceFormat : item.C_InvoiceFormat, 
                      'dropdown',
                      retailerDropdownOptions.invoiceFormat,
                      (val) => {
                        const option = retailerDropdownOptions.invoiceFormat.find((opt: any) => opt.value === String(val));
                        return option ? option.label : (item.C_InvoiceFormat || 'N/A');
                      }
                    )}
                    {renderEditableRetailerField(
                      item.C_Number, 
                      'EDI Format', 
                      'EDI_Format', 
                      customerData.EDI_Format !== undefined ? customerData.EDI_Format : item.EDI_Format, 
                      'dropdown',
                      retailerDropdownOptions.ediFormat,
                      (val) => {
                        const option = retailerDropdownOptions.ediFormat.find((opt: any) => opt.value === String(val));
                    ...
                    {renderEditableRetailerField(item.C_Number, 'Memo', 'C_Memo', customerData.C_Memo || item.C_Memo, 'text')}
                    <Box display="flex" justifyContent="space-between">
                  ...
                  <Box display="flex" flexDirection="column" gap={1}>
                    <Box display="flex" justifyContent="space-between">
                      ...
                      <Typography fontSize={11} fontWeight={500}>
                        {(customerData.LastInvoiceDate || item.LastInvoiceDate) ? new Date(customerData.LastInvoiceDate || item.LastInvoiceDate).toLocaleDateString() : 'N/A'}
                      </Typography>
                    </Box>
                    <Box display="flex" justifyContent="space-between">
                      <Typography fontSize={11} color="text.secondary">Last Payment Date:</Typography>
                      <Typography fontSize={11} fontWeight={500}>
                        {(customerData.LastPaymentDate || item.LastPaymentDate) ? new Date(customerData.LastPaymentDate || item.LastPaymentDate).toLocaleDateString() : 'N/A'}
                      </Typography>
                    </Box>
                    <Box display="flex" justifyContent="space-between">
                      <Typography fontSize={11} color="text.secondary">Last Payment Amount:</Typography>
                      <Typography fontSize={11} fontWeight={500}>${Number(customerData.LastPaymentAmount !== undefined ? customerData.LastPaymentAmount : item.LastPaymentAmount || 0).toFixed(2)}</Typography>
                    </Box>
                    <Box display="flex" justifyContent="space-between">
                      <Typography fontSize={11} color="text.secondary">Delivery Amount:</Typography>
                      <Typography fontSize={11} fontWeight={500}>${Number(customerData.Delivery_Amount !== undefined ? customerData.Delivery_Amount : item.Delivery_Amount || 0).toFixed(2)}</Typography>
                    </Box>
                    <Box display="flex" justifyContent="space-between">
              ...
              >
                ...
                <AccordionDetails sx={{ px: 1, pb: 1, pt: 0.5, flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  {(() => {
                    ...
                    if (!docs) {
                      return <Typography fontSize={13} color="text.secondary">No documents available</Typography>;
                    }
                    
                    const hasDocuments = docs.feinDocument ||
                      docs.salesTaxDoc ||
                      docs.CigTaxDoc ||
                      (docs.attachments && docs.attachments.length > 0) ||
                      (docs.licenseAttachments && docs.licenseAttachments.length > 0);
                    ...
                    return (
                      <Box display="flex" flexDirection="column" gap={1}>
                        {docs.feinDocument && (
                          <Box>
                            <Typography fontSize={13} color="text.secondary" mb={0.5} sx={{ fontWeight: 500 }}>FEIN Document:</Typography>
                            <Typography 
                              component="a"
                        ...
                        {docs.salesTaxDoc && (
                          <Box>
                            <Typography fontSize={13} color="text.secondary" mb={0.5} sx={{ fontWeight: 500 }}>Sales Tax Document:</Typography>
                            <Typography 
                              component="a"
                        ...
                        {docs.CigTaxDoc && (
                          <Box>
                            <Typography fontSize={13} color="text.secondary" mb={0.5} sx={{ fontWeight: 500 }}>Cig Tax Document:</Typography>
                            <Typography 
                              component="a"
                              href={docs.CigTaxDoc}
                              target="_blank"
                        ...
                        {docs.attachments && docs.attachments.length > 0 && (
                          <Box>
                            <Typography fontSize={13} color="text.secondary" mb={0.5} sx={{ fontWeight: 500 }}>Attachments:</Typography>
                            <Box display="flex" flexDirection="column" gap={0.5}>
                              {docs.attachments.map((url: string, idx: number) => (
                                <Typography 
                                  key={idx}
                                  component="a"
                                  href={url}
                        ...
                        {docs.licenseAttachments && docs.licenseAttachments.length > 0 && (
                          <Box>
                            <Typography fontSize={13} color="text.secondary" mb={0.5} sx={{ fontWeight: 500 }}>License Attachments:</Typography>
                            <Box display="flex" flexDirection="column" gap={0.5}>
                              {docs.licenseAttachments.map((url: string, idx: number) => (
                                <Typography 
                                  key={idx}
                                  component="a"
                                  href={url}
            ...
            <Grid size={{ xs: 12, md: 6, lg: 4 }} sx={{ display: 'flex' }}>
              ...
              >
                ...
                <AccordionDetails sx={{ 
                  ...
                  <Box display="flex" flexDirection="column" gap={0.5}>
                    {renderEditableRetailerField(item.C_Number, 'Email Invoice', 'emailInvoice', customerData.emailInvoice !== undefined ? (customerData.emailInvoice === true || customerData.emailInvoice === 1) : (item.emailInvoice !== undefined ? (item.emailInvoice === true || item.emailInvoice === 1) : false), 'toggle')}
                    {renderEditableRetailerField(item.C_Number, 'Email Invoice EDI', 'emailInvoiceEDI', customerData.emailInvoiceEDI !== undefined ? (customerData.emailInvoiceEDI === true || customerData.emailInvoiceEDI === 1) : (item.emailInvoiceEDI !== undefined ? (item.emailInvoiceEDI === true || item.emailInvoiceEDI === 1) : false), 'toggle')}
                    {renderEditableRetailerField(item.C_Number, 'Email Report', 'emailReport', customerData.emailReport !== undefined ? (customerData.emailReport === true || customerData.emailReport === 1) : (item.emailReport !== undefined ? (item.emailReport === true || item.emailReport === 1) : false), 'toggle')}
                    {renderEditableRetailerField(item.C_Number, 'Email Statement', 'emailStatement', customerData.emailStatement !== undefined ? (customerData.emailStatement === true || customerData.emailStatement === 1) : (item.emailStatement !== undefined ? (item.emailStatement === true || item.emailStatement === 1) : false), 'toggle')}
                    {renderEditableRetailerField(item.C_Number, 'Email Promo', 'emailPromo', customerData.emailPromo !== undefined ? (customerData.emailPromo === true || customerData.emailPromo === 1) : (item.emailPromo !== undefined ? (item.emailPromo === true || item.emailPromo === 1) : false), 'toggle')}
                    {renderEditableRetailerField(item.C_Number, 'MSA Accept Promo', 'MSA_AcceptPromo', customerData.MSA_AcceptPromo !== undefined ? (customerData.MSA_AcceptPromo === true || customerData.MSA_AcceptPromo === 1) : (item.MSA_AcceptPromo !== undefined ? (item.MSA_AcceptPromo === true || item.MSA_AcceptPromo === 1) : true), 'toggle')}
                  </Box>
                </AccordionDetails>
              </Accordion>
            </Grid>
                ...
                <AccordionDetails sx={{ px: 1, pb: 1, pt: 0.5, flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <Box display="flex" flexDirection="column" gap={1}>
                    <Box display="flex" justifyContent="space-between">
                      ...
                      <Typography fontSize={11} fontWeight={500}>
                        {(customerData.C_DateCreated || item.C_DateCreated) ? new Date(customerData.C_DateCreated || item.C_DateCreated).toLocaleDateString() : 'N/A'}
                      </Typography>
                    </Box>
