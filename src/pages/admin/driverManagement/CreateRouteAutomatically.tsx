...
import {
  createRetailerLocation,
  getAllOrderForDriver,
...
export type AutoRouteOrderRow = {
  ...
  /** From `Invoice_Total` — optional on persisted Redux rows until refetched */
...
/** Prefer API `invoiceUrl` when present; otherwise static placeholder (same key for future backend). */
export function invoiceUrlFromApi(o: any): string | null {
  const u = o?.invoiceUrl;
  if (u != null && String(u).trim()) return String(u).trim();
  return STATIC_ROUTE_ORDER_INVOICE_URL;
}

const CreateRouteAutomatically: React.FC = () => {
  const theme = useTheme();
  const navigate = useNavigate();
  const location = useLocation();
  const dispatch = useAppDispatch();

  /** Calendar day from Route tab (or back from Optimize) — use for route `deliveryDay`, not raw order date. */
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // API shape:
        // data.routesWithoutStop[] => { Route_Number }
        // data.routesWithStop[]    => { Stop_Number } (fallback if route key missing)
        ...
        // Prefer explicit Route_Number list; if empty, fallback to the other bucket.
    ...
    try {
      ...
      const mapped: AutoRouteOrderRow[] = (Array.isArray(list) ? list : []).map((o: any, idx: number) => {
        ...
        return {
          ...
          customerName: String(customer?.C_Name ?? ""),
          customerAddress: String(
            [customer?.C_Address, customer?.C_City, customer?.C_State, customer?.C_Zip]
              .filter(Boolean)
              .join(", ")
          ),
          route: String(routeNumber || ""),
  ...
  const allSelected =
    eligibleFilteredRows.length > 0 && eligibleFilteredRows.every((r) => !!draftSelectedOrdersById[r.id]);
  const someSelected = eligibleFilteredRows.some((r) => !!draftSelectedOrdersById[r.id]);

  const toggleAll = () => {
    const next: Record<string, AutoRouteOrderRow> = { ...draftSelectedOrdersById };
    if (allSelected) {
  ...
  const handleOptimizeClick = () => {
    ...
    if (missing) {
      ...
      openLocationDialogForRow(missing);
      return;
    }
  ...
  return (
    ...
    >
      ...
      {/* Sub-tabs + legend — sit directly on top of white card (no gap) */}
      <Box
        sx={{
          display: "flex",
      ...
      {/* Above the white card: back, title — on page background (#F4F7F9) */}
      <Button
      ...
      {/* Single white card: filters + table only — flush under sub-tabs */}
      <Paper
        elevation={0}
        sx={{
          mt: 0,
      ...
      >
        ...
        <TableContainer
          sx={{
            overflow: "auto",
            // maxHeight: { xs: "min(70vh, 560px)", md: "min(65vh, 640px)" },
          ...
          <TableHead>
            <TableRow>
              ...
              {[
                ...
                <TableCell
                  key={label}
                  className={label === "Action" ? "sticky-action" : undefined}
                  sx={{
                    py: 1,
                    bgcolor: headerBg,
                    color: "#fff",
            ...
            {loading ? (
              ...
              filteredRows.map((row) => (
                <TableRow
                  ...
                  hover
                  sx={{
                    bgcolor: row.futureDelivery ? futureBg : cardBg,
                  }}
                >
                  ...
                  <TableCell
                    className="sticky-action-body"
                    sx={{ whiteSpace: "nowrap", bgcolor: row.futureDelivery ? futureBg : cardBg }}
                  >
      ...
      <Dialog open={locationDialogOpen} onClose={savingLocation ? undefined : closeLocationDialog} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={closeLocationDialog} disabled={savingLocation} sx={{ textTransform: "none", borderRadius: "8px", boxShadow: "none" }} variant="outlined">
            Cancel
          </Button>
          <Button
            variant="contained"
...
export default CreateRouteAutomatically;
