...
import { getDeliveryCharge, getWarehouseProfile } from "../../../redux/apis/retailer/orderApis";
...
const RetailerTradeShowCart: React.FC = () => {
  ...
  useEffect(() => {
    if (tradeShowId != null) loadCart();
  }, [tradeShowId, loadCart]);

  useEffect(() => {
    dispatch(setTradeShowCartCount(cartItems.length));
  }, [dispatch, cartItems.length]);

  useEffect(() => {
    getDeliveryCharge().then((res: any) => setDeliveryCharge(res?.data ?? 0));
  }, []);
  ...
  useEffect(() => {
    ...
    getWarehouseProfile()
      .then((res: any) => {
        const profile = res?.data?.warehouseProfile;
  ...
  const handleQuantityChange = async (item: CartItemType, change: number) => {
    ...
    try {
      ...
      await updateTradeShowCartItem(item.Product.id, {
  ...
  const handleDeleteItem = async (item: CartItemType) => {
    try {
      await removeTradeShowCartItem(item.Product.id);
  ...
  const handleClearCart = async () => {
    try {
      ...
      setCartItems([]);
      dispatch(setTradeShowCartCount(0));
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const orderPayload = cartItems.map((item: any) => {
        ...
        return {
          Customer_Number: item.Product.Customer_Number ?? auth?.storeDetail?.C_Number,
      ...
      const response: any = await placeTradeShowOrder(payload);
      if (response?.success) {
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
          <Box display="flex" alignItems="center" gap={2}>
            ...
            <Button
              ...
              color="primary"
              size="small"
              onClick={() => navigate("/retailer/trade-show")}
...
export default RetailerTradeShowCart;
