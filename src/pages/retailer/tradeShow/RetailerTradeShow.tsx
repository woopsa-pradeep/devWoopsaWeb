...
import { setTradeShowCartCount } from "../../../redux/slices/retailerTradeShowCartSlice";
import {
  getRetailerTradeShow,
  getTradeShowItems,
  addToTradeShowCart,
  updateTradeShowCartItem,
  removeTradeShowCartItem,
  clearTradeShowCart,
  getTradeShowCartItems,
  GetTradeShowItemsParams,
} from "../../../redux/apis/retailer/tradeShowApis";
import { getSalesCategoryPriceClassByCustomer, getSalesCategoryByCustomer } from "../../../redux/apis/retailer/orderApis";
import { useShowPrepaidTax, calculateDisplayPrice } from "../../../utils/prepaidTaxDisplayUtils";
import TradeShowOrderDetails, { TradeShowOrderItem } from "./TradeShowOrderDetails";

// API product shape (same as Order inventory)
...
const RetailerTradeShow: React.FC = () => {
  ...
  // Fetch current trade show
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res: any = await getRetailerTradeShow();
        // API: { success, message, data: { id, name, tradeShowDate, ... } }
  ...
  // Filter options
  const customerId = auth?.storeDetail?.C_Number;
  useEffect(() => {
  ...
  const getItems = useCallback(async () => {
    if (tradeShowId == null) return;
    setLoading(true);
    setError(null);
    try {
      const params: GetTradeShowItemsParams = {
        page: currentPage,
        limit: pageSize,
      ...
      const response: any = await getTradeShowItems(params);
  ...
  }, [tradeShowId, currentPage, pageSize, debouncedSearchTerm, masterSearchTerm, salesCategory, priceClass, userSalesCategory, stateFromStore]);

  useEffect(() => {
    if (tradeShowId != null) getItems();
  }, [tradeShowId, getItems]);
  ...
  const applyTradeShowCartResponse = useCallback((items: any[]) => {
    ...
    dispatch(setTradeShowCartCount(order.length));
  }, [dispatch]);

  const loadTradeShowCart = useCallback(async () => {
    if (tradeShowId == null || cartLoadedRef.current) return;
    try {
      const response: any = await getTradeShowCartItems(tradeShowId);
  ...
  }, [tradeShowId, applyTradeShowCartResponse]);

  /** Refresh cart from server (calls GET /api/retailer/tradeShow/items/:tradeshowId) - same as Order page after add/edit/delete */
  ...
  useEffect(() => {
    if (tradeShowId != null) loadTradeShowCart();
    return () => { cartLoadedRef.current = false; };
  }, [tradeShowId, loadTradeShowCart]);
  ...
  const handleQuantityChange = async (id: string, change: number) => {
    ...
    if (newQty <= 0) {
      ...
      try {
        if (current?.productId) await removeTradeShowCartItem(current.productId);
    ...
    quantityDebounceRef.current[id] = setTimeout(async () => {
      ...
      try {
        ...
        if (current?.productId) {
          await updateTradeShowCartItem(current.productId, payload);
        } else {
          await addToTradeShowCart({ Item_Number: parseInt(id, 10), ...payload });
        }
        await refreshTradeShowCart();
      } finally {
        setProductLoading(id, false);
  ...
  const handleRemoveItem = async (id: string) => {
    ...
    try {
      ...
      if (productId) await removeTradeShowCartItem(productId);
      await refreshTradeShowCart();
    } finally {
      setProductLoading(id, false);
    }
  ...
  const handleContinueOrder = () => navigate("/retailer/trade-show/cart");
...
export default RetailerTradeShow;
