...
import { updateCartItem, removeFromCart, getDeliveryCharge, placeOrder, getWarehouseProfile, clearCart as clearCartApi, getRecommendations, getInventoryItems, addToCart } from '../../../redux/apis/retailer/orderApis';
...
const CartPage: React.FC = () => {
  ...
  const loadRecommendedProducts = useCallback(async () => {
    ...
    try {
      ...
      const userId = addresses?.storeDetail?.C_Number?.toString() || null;
      ...
      if (response?.data?.finalProductList) {
        ...
        const transformedProducts = response.data.finalProductList.map((apiProduct: any) => ({
          ...
          showTheInventoryStock: apiProduct.showTheInventoryStock || false,
          showLowStock: apiProduct.showLowStock || false,
        }));
  ...
  // Load recommendations when cart gets new items (avoid quantity-only updates)
  useEffect(() => {
    ...
    // If cart gained a new item, refresh recommendations
  ...
  const handleQuantityChange = async (item: CartItem, change: number) => {
    ...
    try {
      ...
      if (!item.Product.Customer_Number) {
    ...
    try {
      ...
      if (product.hasQtyDiscount && product.qtyDiscount) {
        ...
        setSelectedDiscountProduct({
          ...
          Product: {
            id: 0,
            Customer_Number: addresses?.storeDetail?.C_Number || 0,
