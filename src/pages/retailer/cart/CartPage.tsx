...
import { updateCartItem, removeFromCart, getDeliveryCharge, placeOrder, getWarehouseProfile, clearCart as clearCartApi } from '../../../redux/apis/retailer/orderApis';
...
// Interface for warehouse profile data
...
const CartPage: React.FC = () => {
  ...
  const  addresses  = useSelector((state: any) => state.auth)
      const shippingAddress = `${addresses?.storeDetail?.C_Address}, ${addresses?.storeDetail?.C_City}, ${addresses?.storeDetail?.C_State}`;
  ...
  const loadWarehouseProfile = async () => {
    ...
    try {
      const response: any = await getWarehouseProfile();
      if (response?.data) {
        const profile = response.data.warehouseProfile;
        setWarehouseProfile(profile);
        
        // Set default shipping method based on what's available
        if (!profile.allowShipping && profile.storePickup) {
          setShippingMethod('pickup');
        } else if (profile.allowShipping && !profile.storePickup) {
          setShippingMethod('delivery');
        }
        // If both are available, keep the current default (delivery)
        // If neither is available, keep current but user will see error message
      }
    } catch (error) {
  ...
  const handleQuantityChange = async (item: CartItem, change: number) => {
    ...
    try {
      ...
      const originalBasePrice = Number((Number(item.Product.originalPrice) + Number(item.Product.Tax_Rate)).toFixed(2));
      ...
      await updateCartItem(item.Product.id, {
        Qty: newQuantity,
        Price: Number((Number(item.Product.originalPrice) - (item.qtyDiscount.price - finalPrice)).toFixed(2)), // Ensure price is a number with 2 decimal places        
  ...
  const handleDeleteItem = async (item: CartItem) => {
    try {
      ...
      // Refresh cart items from server to update Redux state
      await loadCartItems();
    } catch (error) {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const response: any = await placeOrder(payload);
      if(response?.success){
  ...
  useEffect(() => {
    const fetchDeliveryCharge = async () => {
      const response: any = await getDeliveryCharge();
      ...
      try {
        ...
        setTimeout(async () => {
          try {
                    await updateCartItem(item.Product.id, {
          Qty: quantity,
          Price: Number((Number(item.Product.originalPrice) - (item.qtyDiscount.price - discountedPrice)).toFixed(2)), // Ensure price is a number with 2 decimal places
      ...
      try {
        ...
        setTimeout(async () => {
          try {
                    await updateCartItem(item.Product.id, {
          Qty: quantity,
          Price: Number(Number(item.Product.Price).toFixed(2)), // Ensure price is a number with 2 decimal places
    ...
    <Box sx={{ p: 2, pt: 0 }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, lg: 4 }}>
          <Grid container spacing={3}>
            {cartItems[0]?.showWithOutPrice == false && <Grid size={12}>
      ...
      <OrderCelebration
        ...
        onClose={() => {
          ...
          navigate(`/retailer/order/details/${orderNumber}`);
        }}
