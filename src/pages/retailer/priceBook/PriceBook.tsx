...
import { getPriceBookInventoryItems, addToCart, updateCartItem, removeFromCart, clearCart, InventoryParams, getSalesCategoryPriceClassByCustomer, getSalesCategoryByCustomer } from '../../../redux/apis/retailer/orderApis';
...
import { fetchCartItems, setCartCount } from '../../../redux/slices/cartSlice';
...
const Order = () => {
  ...
  useEffect(() => {
    dispatch(setCartCount(Object.keys(orderItems).length));
  }, [dispatch, orderItems]);
  ...
  const getInventoryData = useCallback(async () => {
    ...
    try {
      ...
      const response:any = await getPriceBookInventoryItems(params, abortController.signal);
  ...
  const processQrCode = async (upcCode: string) => {
    try {
      ...
      const response: any = await getPriceBookInventoryItems(params);
