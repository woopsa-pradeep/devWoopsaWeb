...
import { getNewItem, getDiscountedItems, getPromotedItems } from '../apis/dashboardApis';
...
interface ApiProduct {
  ...
  totalPrice: number;
  UnitOunces: any;
  showLowStockToSalesRep: boolean;
  showTheInventoryStockToSalesRep: boolean;
  showWithOutPriceToSalesRep: boolean;
...
const transformApiProduct = (apiProduct: ApiProduct): DashboardProduct => {
  ...
  const showTheInventoryStock = apiProduct.showTheInventoryStockToSalesRep || false;
  const inventoryOnHand = apiProduct.Inventory_OnHand || 0;
  const showWithOutPrice = apiProduct.showWithOutPriceToSalesRep || false;
  const allowToOrder = apiProduct.allowToOrderSalesRep !== false; // Default to true if not specified
  const showLowStock = apiProduct.showLowStockToSalesRep || false;
...
export const fetchSalesNewItems = createAsyncThunk(
  'salesDashboard/fetchNewItems',
...
export const fetchSalesDiscountedItems = createAsyncThunk(
  'salesDashboard/fetchDiscountedItems',
  async (
    params: { role: string; c_number: string },
    { rejectWithValue }
  ) => {
    try {
      // Validate parameters before making API call
      if (!params.role || !params.c_number || params.role === 'undefined' || params.c_number === 'undefined') {
        return rejectWithValue('Invalid parameters: role and c_number are required');
      }
      
      const response: any = await getDiscountedItems(params.role, params.c_number);
      const apiProducts = response?.data?.finalProductList || [];
      return apiProducts.map((product: ApiProduct) => transformApiProduct(product));
    } catch (error: any) {
...
export const fetchSalesPromotedItems = createAsyncThunk(
  'salesDashboard/fetchPromotedItems',
  async ({ role, c_number }: { role: string; c_number: string }, { rejectWithValue }) => {
    try {
      // Validate parameters before making API call
      if (!role || !c_number || role === 'undefined' || c_number === 'undefined') {
      ...
      const response: any = await getPromotedItems(role, c_number);
      const apiProducts = response?.data?.promotedItemsList || [];
      return apiProducts.map((product: ApiProduct) => transformApiProduct(product));
    } catch (error: any) {
...
const salesDashboardSlice = createSlice({
  ...
  initialState,
  reducers: {
    clearSalesDashboardData: (state) => {
      state.newItems = [];
      state.discountedItems = [];
      state.popularItems = [];
      state.error = null;
    },
    setSalesDashboardError: (state, action: PayloadAction<string>) => {
  ...
  extraReducers: (builder) => {
    builder
      // New Items
      .addCase(fetchSalesNewItems.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchSalesNewItems.fulfilled, (state, action) => {
        state.loading = false;
        if (action.payload) {
          state.newItems = action.payload;
        }
      })
      .addCase(fetchSalesNewItems.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Failed to fetch new items';
      })
      // Discounted Items
      .addCase(fetchSalesDiscountedItems.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchSalesDiscountedItems.fulfilled, (state, action) => {
        state.loading = false;
        if (action.payload) {
          state.discountedItems = action.payload;
        }
      })
      .addCase(fetchSalesDiscountedItems.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.message || 'Failed to fetch discounted items';
      })
      // Popular Items
      .addCase(fetchSalesPromotedItems.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchSalesPromotedItems.fulfilled, (state, action) => {
        state.loading = false;
        if (action.payload) {
          state.popularItems = action.payload;
        }
      })
      .addCase(fetchSalesPromotedItems.rejected, (state, action) => {
        state.loading = false;
...
export const { clearSalesDashboardData, setSalesDashboardError } = salesDashboardSlice.actions;
