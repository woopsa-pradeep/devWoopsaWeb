...
import {
  ...
  updateWebCategory,
  createWebPriceClass,
  getAllWebPriceClasses,
  deleteWebPriceClass,
  updateWebPriceClass,
  getProductListBySearch,
  associateProductsWithWebView,
  getProductsByWebView,
  createWebViewForSection,
  getQuickLinks,
  createQuickLink,
  updateQuickLink,
  deleteQuickLink,
...
import {
  ...
  getPriceClassList,
} from "../../../redux/apis/distrubutor/listApis";
...
const WebView: React.FC<WebViewPermProps> = ({ canAdd = true, canEdit: _canEdit = true }) => {
  ...
  const [updatePriceClassId, setUpdatePriceClassId] = useState<number | null>(
    null,
  );
  const [updateQuickLinkId, setUpdateQuickLinkId] = useState<number | null>(
    null,
  );

  // Delete confirmation states
  const [deleteCategoryModalOpen, setDeleteCategoryModalOpen] = useState(false);
  const [deletePriceClassModalOpen, setDeletePriceClassModalOpen] =
    useState(false);
  const [deleteQuickLinkModalOpen, setDeleteQuickLinkModalOpen] =
    useState(false);
  const [categoryToDelete, setCategoryToDelete] = useState<Category | null>(
    null,
  );
  const [priceClassToDelete, setPriceClassToDelete] =
    useState<PriceClass | null>(null);
  const [quickLinkToDelete, setQuickLinkToDelete] = useState<QuickLink | null>(
    null,
  );

  // Status change confirmation modal states
  const [statusChangeModalOpen, setStatusChangeModalOpen] = useState(false);
  const [quickLinkToUpdateStatus, setQuickLinkToUpdateStatus] =
    useState<QuickLink | null>(null);
  const [newStatus, setNewStatus] = useState<boolean>(false);
  const [statusUpdateLoading, setStatusUpdateLoading] = useState(false);

  // ShowInWeb change confirmation modal states
  const [showInWebChangeModalOpen, setShowInWebChangeModalOpen] =
    useState(false);
  const [quickLinkToUpdateShowInWeb, setQuickLinkToUpdateShowInWeb] =
    useState<QuickLink | null>(null);
  ...
  const [selectedWebViewImage, setSelectedWebViewImage] =
    useState<WebViewImage | null>(null);
  ...
  const fetchWebViewImages = useCallback(async () => {
    ...
    try {
      ...
      if (!sections || !Array.isArray(sections)) {
        console.error(
          "Invalid sections state in fetchWebViewImages:",
          sections,
        );
      ...
      const enhancedWebViewImages = await Promise.all(
        webViewImages.map(async (image) => {
          if (image.section === "middle" || image.section === "bottom") {
            try {
              const productsResponse: any = await getProductsByWebView(
                image.id.toString(),
              );
              let products: any[] = [];
              ...
              console.error(
                `Error fetching products for image ${image.id}:`,
                err,
              );
              return image;
            }
      ...
      const groupedImages = sections.map((section) => {
        if (!section || !section.sectionKey) {
          console.error(
            "Invalid section found in fetchWebViewImages:",
            section,
          );
          return {
        ...
        return {
          ...section,
          images: Array.isArray(enhancedWebViewImages)
            ? enhancedWebViewImages
                .filter(
                  (img: WebViewImage) =>
                    img.section === section.sectionKey && img.isActive,
                )
      ...
      setError(
        err.response?.data?.message || "Failed to fetch web view images",
      );
  ...
  const fetchWebCategories = async () => {
    ...
    try {
      const response: any = await getAllWebCategories(
  ...
  const fetchWebPriceClasses = async () => {
    ...
    try {
      const response: any = await getAllWebPriceClasses(
        priceClassesPagination.page,
        priceClassesPagination.limit,
      );
      // Handle the actual API response structure
  ...
  const handleCategorySubmit = async () => {
    ...
    try {
      const selectedCat = salesCategories.find(
        (cat) => cat.Sales_Category == selectedCategory,
      );
  ...
  const handlePriceClassSubmit = async () => {
    ...
    try {
      const selectedPriceClassItem = priceClassList.find(
        (pc) => pc.Price_Class == selectedPriceClass,
      );
      const formData = new FormData();
      if (priceClassImage) {
      ...
      toast.error(
        err.response?.data?.message || "Failed to save price class image",
      );
    } finally {
  ...
  const handleProductModalOpen = async (image: WebViewImage) => {
    ...
    if (image.id !== -1) {
      ...
      if (
        (image as any).products &&
        Array.isArray((image as any).products) &&
        (image as any).products.length > 0
      ) {
  ...
  const handleProductSelection = (product: Product) => {
    const isSelected = selectedProducts.some(
      (p) => p.Item_Number === product.Item_Number,
    );
    if (isSelected) {
      setSelectedProducts((prev) =>
        prev.filter((p) => p.Item_Number !== product.Item_Number),
      );
    } else {
    ...
    try {
      ...
      if (selectedWebViewImage.id === -1) {
        ...
        const currentSection = sections.find(
          (s) => s.sectionKey === selectedWebViewImage.section,
        );
      ...
      // Update the products
      await associateProductsWithWebView(
        selectedWebViewImage.id.toString(),
        productIds,
      );
  ...
  const handleViewModalOpen = async (bannerData: any) => {
    ...
    try {
      const response: any = await getProductsByWebView(
  ...
  const confirmDeleteCategory = async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "Failed to delete category image",
      );
    } finally {
  ...
  const confirmDeletePriceClass = async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "Failed to delete price class image",
      );
    } finally {
  ...
  const confirmStatusChange = async () => {
    ...
    try {
      ...
      toast.success(
        `Quick link status updated to ${newStatus ? "Active" : "Inactive"}`,
      );
      await fetchQuickLinks();
      ...
      toast.error(
        err.response?.data?.message || "Failed to update quick link status",
      );
    } finally {
  ...
  const confirmShowInWebChange = async () => {
    ...
    try {
      ...
      toast.success(
        `Quick link show in web updated to ${newShowInWeb ? "Yes" : "No"}`,
      );
      await fetchQuickLinks();
      ...
      toast.error(
        err.response?.data?.message ||
          "Failed to update quick link show in web",
      );
    } finally {
  ...
  const handleCategoriesPageChange = async (page: number) => {
    ...
    await getAllWebCategories(page, newPagination.limit).then(
      (response: any) => {
        if (response?.data?.data?.webCategories) {
          setCategories(response.data.data.webCategories);
          setCategoriesPagination(
            response.data.data.pagination || newPagination,
          );
        }
      },
    );
  ...
  const handlePriceClassesPageChange = async (page: number) => {
    ...
    await getAllWebPriceClasses(page, newPagination.limit).then(
      (response: any) => {
        if (response?.data?.data?.webPriceClasses) {
          setPriceClasses(response.data.data.webPriceClasses);
          setPriceClassesPagination(
  ...
  const handleProductSelectionForNewImage = async (sectionIndex: number) => {
    ...
    if (!section || !section.sectionKey) {
      console.error(
        "Invalid section found in handleProductSelectionForNewImage:",
        section,
      );
  ...
  const handleNextPage = useCallback(
    (sectionKey: string, totalImages: number) => {
  ...
  const renderImageSlot = useCallback(
    (image: WebViewImage | null, sectionIndex: number, imageIndex: number) => {
      const hasImage = image && image.image_url;
      const isUploading =
        uploadingImage?.sectionIndex === sectionIndex &&
        uploadingImage?.imageIndex === imageIndex;
      const isUploadingNew =
        uploadingImage?.sectionIndex === sectionIndex &&
        !uploadingImage?.imageIndex &&
        !hasImage;
      const isImageLoaded = hasImage
      ...
      return (
        <Box sx={{ p: 3, position: "relative" }}>
          ...
          {hasMoreThan4Images && (
            <>
              ...
              {canGoNext && (
                <IconButton
                  onClick={() =>
                    handleNextPage(section.sectionKey, section.images.length)
                  }
                  sx={{
                    position: "absolute",
                    right: 8,
          ...
          {hasMoreThan4Images && (
            ...
            >
              {Array.from(
                { length: Math.ceil(section.images.length / 4) },
                (_, pageIndex) => (
                  <Box
                    key={pageIndex}
                    sx={{
                      ...
                      borderRadius: "50%",
                      backgroundColor:
                        pageIndex === currentPage ? "primary.main" : "grey.300",
                      cursor: "pointer",
                      transition: "all 0.2s ease-in-out",
                      "&:hover": {
                        backgroundColor:
            ...
            >
              {isUploading ? (
                ...
                >
                  ...
                  <Typography
                    variant="h5"
                    sx={{ textAlign: "center", opacity: 0.8, fontWeight: 500 }}
                  >
                    No Banner Created
                  </Typography>
                  <Typography
                    variant="body1"
                    sx={{ textAlign: "center", opacity: 0.6, maxWidth: 400 }}
                  >
                    Click "Create Banner" to upload a banner image and associate
                    it with products
                  </Typography>
                </Box>
              )}
              ...
              render: (row: any) => (
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <SwitchInput
                    ...
                    onChange={(checked: boolean) =>
                      handleShowInWebChange(row, checked)
                    }
                    size="small"
                    sx={{ mb: 0 }}
                    isShowLabel={false}
    ...
    <Box sx={{ p: 1 }}>
              ...
              >
                ...
                >
                  ...
                  <Box sx={{ display: "flex", gap: 1 }}>
                    ...
                    {(section.sectionKey === "middle" ||
                      section.sectionKey === "bottom") &&
                      section.images &&
                      section.images.length === 0 && (
                        <CustomButton
                          appearance="filled"
                          onClick={() =>
                            handleProductSelectionForNewImage(sectionIndex)
                          }
                          size="small"
                          fullWidth={false}
                          sx={{ mt: 0 }}
                ...
                {/* Image Grid */}
                {section.sectionKey === "header"
                  ? renderImageGrid(section, sectionIndex)
                  : renderBannerSection(section, sectionIndex)}
              </Paper>
            );
          })
        ) : (
      ...
      >
        <DialogTitle
          ...
          sx={{ color: "primary.main", fontWeight: 500, fontSize: 18, pb: 1 }}
        >
          Confirm Deletion
        </DialogTitle>
        <DialogContent>
          ...
          <Box sx={{ mt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
            {isUpdatingCategory && (
              <Box sx={{ mb: 2 }}>
                ...
                {categories.find((cat) => cat.id === updateCategoryId)
                  ?.image ? (
                  <Box
                    component="img"
                    src={
            ...
            <FileUploadInput
              ...
              helperText={
                isUpdatingCategory
                  ...
                  : "Select an image file for this category"
              }
            />
          </Box>
        </DialogContent>
      ...
      {/* Price Class Modal */}
      <Dialog
        open={priceClassModalOpen}
        onClose={handlePriceClassModalClose}
        maxWidth="sm"
        fullWidth
      >
        <DialogTitle>
          {isUpdatingPriceClass
            ? "Update Price Class Image"
            : "Add Price Class Image"}
        </DialogTitle>
        <DialogContent>
          <Box sx={{ mt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
            {isUpdatingPriceClass && (
              <Box sx={{ mb: 2 }}>
                ...
                {priceClasses.find((pc) => pc.id === updatePriceClassId)
                  ?.image ? (
                  <Box
                    component="img"
                    src={
            ...
            <SelectInput
              ...
              onChange={(e) =>
                setSelectedPriceClass(e.target.value as number | "")
              }
            ...
            <FileUploadInput
              label={
                isUpdatingPriceClass
                  ? "Upload New Price Class Image (optional)"
                  : "Upload Price Class Image"
              }
              ...
              helperText={
                isUpdatingPriceClass
                  ? "Select a new image file to replace the current one"
                  : "Select an image file for this price class"
              }
            />
          </Box>
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
          <CustomButton
            onClick={handlePriceClassModalClose}
            appearance="outlined"
            fullWidth={false}
            sx={{ mt: 0 }}
          >
        ...
        <DialogContent>
          <Box sx={{ mt: 2 }}>
            <Typography variant="body1" color="text.primary" sx={{ mb: 2 }}>
              ...
              {quickLinkToUpdateStatus?.name}" to{" "}
      ...
      <Dialog
        open={showInWebChangeModalOpen}
        onClose={() => setShowInWebChangeModalOpen(false)}
        maxWidth="sm"
        fullWidth
      >
        ...
        <DialogContent>
          <Box sx={{ mt: 2 }}>
            <Typography variant="body1" color="text.primary" sx={{ mb: 2 }}>
              Are you sure you want to change the show in web setting for "
              {quickLinkToUpdateShowInWeb?.name}" to{" "}
              <strong>{newShowInWeb ? "Yes" : "No"}</strong>?
            </Typography>
            <Typography variant="body2" color="text.secondary">
              This will update whether the quick link is displayed on the web
              interface.
            </Typography>
          </Box>
        </DialogContent>
      ...
      >
          ...
          <Box sx={{ mt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <SwitchInput
                checked={
                  isUpdatingQuickLink
                    ? quickLinkToUpdateStatus?.status || false
                    : true
                }
                onChange={(checked: boolean) => {
                  if (isUpdatingQuickLink && quickLinkToUpdateStatus) {
                    setQuickLinkToUpdateStatus({
                      ...quickLinkToUpdateStatus,
                      status: checked,
                    });
                  }
                }}
              ...
              <Box>
                ...
                <Typography variant="caption" color="text.secondary">
                  {isUpdatingQuickLink
                    ? quickLinkToUpdateStatus?.status
                      ? "Active"
                      : "Inactive"
                    : "Active (default)"}
                </Typography>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <SwitchInput
                checked={
                  isUpdatingQuickLink
                    ? quickLinkToUpdateStatus?.showInWeb || false
                    : true
                }
                onChange={(checked: boolean) => {
                  if (isUpdatingQuickLink && quickLinkToUpdateStatus) {
              ...
              <Box>
                ...
                <Typography variant="caption" color="text.secondary">
                  {isUpdatingQuickLink
                    ? quickLinkToUpdateStatus?.showInWeb
                      ? "Yes"
                      : "No"
                    : "Yes (default)"}
                </Typography>
        ...
        <DialogActions sx={{ p: 2 }}>
          <CustomButton
            onClick={handleQuickLinkModalClose}
            appearance="outlined"
            fullWidth={false}
            sx={{ mt: 0 }}
          >
      ...
      >
        <DialogTitle>
          {selectedWebViewImage?.id === -1
            ? "Create Banner with Products"
            : "Edit Banner Image & Products"}
            ...
            >
              ...
              {selectedWebViewImage?.id !== -1 && selectedWebViewImage && (
                <Box>
                  ...
                  >
                    You can modify both the image and products associated with
                    this banner.
                  </Typography>
                  ...
                  {selectedWebViewImage.products &&
                    selectedWebViewImage.products.length > 0 && (
                      <Box
                        sx={{
                          mt: 2,
                          p: 2,
                      ...
                      >
                        ...
                        >
                          Current Products (
                          {selectedWebViewImage.products.length}):
                        </Typography>
                        {selectedWebViewImage.products.map(
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          <CustomButton
            ...
            disabled={
              ...
              selectedProducts.length === 0 ||
              (selectedWebViewImage?.id === -1 && !newImageFile)
            }
          >
            {productSubmitLoading ? (
              <>
        ...
        <DialogTitle>
          ...
          {selectedBannerData && (
            <Typography
              ...
              sx={{ mt: 1, fontWeight: "normal" }}
            >
              {selectedBannerData.section === "middle"
                ? "Middle Banner"
                : "Bottom Banner"}
            </Typography>
          )}
        ...
        <DialogContent>
          {selectedBannerData ? (
            ...
            >
              ...
              {/* Associated Products */}
              {selectedBannerData.products &&
                selectedBannerData.products.length > 0 && (
                  <Box>
                    <Typography
                      variant="subtitle2"
                      color="text.primary"
                    ...
                    >
                      {selectedBannerData.products.map(
                        (product: any, index: number) => (
                          <Box
                            key={product.Item_Number}
                            sx={{
                              p: 2,
                              borderBottom:
                                index < selectedBannerData.products.length - 1
                                  ? "1px solid #f0f0f0"
                                  : "none",
