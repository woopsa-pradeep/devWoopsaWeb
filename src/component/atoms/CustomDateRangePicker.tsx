...
import { Box, FormHelperText, SxProps, Typography, IconButton, Tooltip } from "@mui/material";
...
const CustomDateRangePicker: React.FC<Props> = ({ 
  ...
  const validStartDate = startDate && dayjs.isDayjs(startDate) ? startDate : null;
  const validEndDate = endDate && dayjs.isDayjs(endDate) ? endDate : null;

  return (
    <Box sx={sx}>
      {/* External Label */}
  {isLabel &&    <Box display="flex" justifyContent="space-between" alignItems="center" mb="5px">
        <Typography
          fontSize={14}
          fontWeight={600}
          sx={{ opacity: "70%" }}
