...
interface LossQtyReportModalProps {
  open: boolean;
  onClose: () => void;
  groupBy: 'item' | 'retailer'; // 'item' for Product page, 'retailer' for Retailer page
}

const LossQtyReportModal: React.FC<LossQtyReportModalProps> = ({ open, onClose, groupBy }) => {
  ...
  const getDefaultFields = useCallback((groupByValue: 'item' | 'retailer'): { [key: string]: boolean } => {
    if (groupByValue === 'item') {
      ...
      // For 'retailer' (customer): Only Item_Number, Description, Loss_Qty, Ext_Price, Ext_Loss
      return {
        Item_Number: true,
        Description: true,
        Loss_Qty: true,
  ...
  // Pagination state for chunked data fetching
  ...
  // Reset when modal opens/closes and fetch data
  useEffect(() => {
    if (open) {
      fetchFilterOptions();
      setCurrentChunk(0);
      ...
      setReportData([]);
      setFilteredData([]);
      // Call API when modal opens (can call without dates, like original)
      fetchReportData();
  ...
  // Fetch data when dates change (only after initial load)
  useEffect(() => {
  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      const firstResponse = await getShortShipmentReport(params) as any;
      const firstRows = firstResponse?.data?.data?.rows || firstResponse?.data?.rows || firstResponse?.rows || [];
      ...
      if (firstRows.length < CHUNK_SIZE) {
        setReportData(firstRows);
        setFilteredData(firstRows);
        setLoading(false);
        return;
      }

      // For large datasets, we'll fetch in chunks
      // Since the API doesn't support pagination, we'll process the data in chunks on the frontend
      const chunks: LossQtyReportRow[][] = [];
      for (let i = 0; i < firstRows.length; i += CHUNK_SIZE) {
        chunks.push(firstRows.slice(i, i + CHUNK_SIZE));
      }

      setAllChunks(chunks);
      setCurrentChunk(0);
      setReportData(chunks[0] || []);
      setFilteredData(chunks[0] || []);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (selectedOTPTypes.length > 0 && allData.length > 0 && 'OTP_Number' in allData[0]) {
      filtered = filtered.filter(row => 
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const selectedFieldKeys = Object.keys(selectedFields).filter(key => selectedFields[key]);
      const totalColumns = 2 + selectedFieldKeys.length;
      ...
      doc.text(`Grouped by: ${groupBy === 'item' ? 'Item' : 'Retailer'}`, margin, yPosition);
      yPosition += 6;

      const headers = selectedFieldKeys.map(key => FIELD_LABELS[key] || key);
      
      const availableWidth = pageWidth - (margin * 2);
      const selectedFieldWidth = orientation === 'portrait' 
        ? availableWidth / selectedFieldKeys.length
        : 30;
      const colWidths = selectedFieldKeys.map(() => selectedFieldWidth);

      const tableData: any[][] = [];

      getGroupedData.forEach(item => {
  ...
  return (
    <CommonModal
      ...
      onClose={onClose}
      size="xxl"
      title={`Loss Quantity Report - ${groupBy === 'item' ? 'Item Wise' : 'Retailer Wise'}`}
    >
      <Box sx={{ 
        ...
        >
          ...
          <CustomButton
            ...
            disabled={generatingReport || generatingPDF || filteredData.length === 0}
...
export default LossQtyReportModal;
